...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        function updateFileCounter(s, obj) {
            if(s.showFileCounter) {
                ...
                $(obj.container).find(".ajax-file-upload-filename").each(function (i, items) {
                    // 最大上传数为1时 这里有bug
                    // 改：if (s.fileCounterStyle) {}
                    if (s.fileCounterStyle) {
                        var arr = $(this).html().split(s.fileCounterStyle);
                        var fileNum = parseInt(arr[0]) - 1; //decrement;
                        var name = s.fileCounterStyle + arr[1];
                        $(this).html(name);
                        count--;
                    }
                });
		...
		function defaultProgressBar(obj,s)
		{
            ...
            this.abort = $("<div class='fa fa-times'></div>").appendTo(this.statusbar).hide();
            this.cancel = $("<div>" + s.cancelStr + "</div>").appendTo(this.statusbar).hide();
            this.done = $("<div>" + s.doneStr + "</div>").appendTo(this.statusbar).hide();
            this.download = $("<div>" + s.downloadStr + "</div>").appendTo(this.statusbar).hide();
            this.del = $("<div class='fa fa-times'></div>").appendTo(this.statusbar).hide();
